...
import { getByProject } from "@/lib/api/resourceForecast";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const weeksCount = parseInt(sp.get("weeks_count") ?? "8", 10);
    ...
    const data = await withOrgContext(userId, async (db) => {
      ...
      return getByProject(db, orgId, weekStart, weeksCount, projectId);
    });
