...
export type ProjectsPageInitialData = {
  projects: Project[];
  milestoneCounts: Record<string, number>;
  taskProgress: Record<string, { done: number; total: number }>;
  taskDeadlines: { projectId: string; projectName: string; taskId: string; taskTitle: string; dueDate: string }[];
  projectMembers: Record<string, ProjectMember[]>;
  health: Record<string, HealthSnapshot>;
};

export function ProjectsPageClient({ initial }: { initial?: ProjectsPageInitialData }) {
  const { selectedOrgId, can, loading: orgLoading } = useOrg();
  const [projects, setProjects] = useState<Project[]>(initial?.projects ?? []);
  const [milestoneCounts, setMilestoneCounts] = useState<Record<string, number>>(initial?.milestoneCounts ?? {});
  const [taskProgress, setTaskProgress] = useState<Record<string, { done: number; total: number }>>(initial?.taskProgress ?? {});
  const [projectMembers, setProjectMembers] = useState<Record<string, ProjectMember[]>>(initial?.projectMembers ?? {});
  const [health, setHealth] = useState<Record<string, HealthSnapshot>>(initial?.health ?? {});
