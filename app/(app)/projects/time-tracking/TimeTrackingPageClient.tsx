...
export type TimeTrackingInitialData = {
  entries: TimeEntry[];
  summary: Summary;
...
export function TimeTrackingPageClient({ initial }: { initial?: TimeTrackingInitialData }) {
  ...
  const [submission, setSubmission] = useState<Submission | null>(initial?.submission ?? null);
  ...
  const skippedInitialData = useRef(!!initial);
  useEffect(() => {
    if (skippedInitialData.current) {
      skippedInitialData.current = false;
      return;
    }
    loadData();
  }, [loadData]);
