...
export type Milestone = { id: string; name: string; dueDate: string | null; approvedAt: string | null };
export type Sprint = { id: string; name: string; status: string; startDate: string | null; endDate: string | null };

const TABS = ["Overview", "Tasks", "Team", "Files", "Activity", "Settings"] as const;
type Tab = (typeof TABS)[number];

export type ProjectDetailInitialData = {
  project: Project;
...
export default function ProjectDetailPageClient({
  ...
  initial?: ProjectDetailInitialData;
}) {
  const { id } = use(params);
  ...
  const [peopleById, setPeopleById] = useState<Record<string, { fullName: string }>>(initial?.peopleById ?? {});
  ...
  const skippedInitialPeople = useRef(!!initial);
  useEffect(() => {
    if (!selectedOrgId) return;
    if (skippedInitialPeople.current) {
      skippedInitialPeople.current = false;
      return;
    }
            ...
            <>
              <div className="mt-3 flex items-baseline justify-between text-body">
                <span className="text-neutral-600">
                  Spent <span className="font-medium text-neutral-950">${budgetSpent.toLocaleString("en-US")}</span>
                </span>
                <span className="text-neutral-600">
                  of <span className="font-medium text-neutral-950">${budgetAllocated.toLocaleString("en-US")}</span>
                </span>
              </div>
          ...
          <ul className="mt-3 divide-y divide-neutral-200">
            {activity.slice(0, 5).map((a) => (
              <li key={a.id} className="flex items-center justify-between py-2 text-body">
                ...
                <span className="text-small text-neutral-500">{new Date(a.createdAt).toLocaleString("en-US")}</span>
              </li>
            ))}
          </ul>
        )}
  ...
  return (
    <Card padding="sm" className="space-y-2">
      ...
      <div className="flex flex-wrap gap-x-6 gap-y-1 text-body">
        <span className="text-neutral-600">
          Allocated: <span className="font-medium text-neutral-950">{project.budgetAllocated != null ? `$${allocated.toLocaleString("en-US")}` : "Not set"}</span>
        </span>
        <span className="text-neutral-600">
          Spent: <span className="font-medium text-neutral-950">{project.budgetSpent != null ? `$${spent.toLocaleString("en-US")}` : "Not set"}</span>
        </span>
      </div>
    ...
    <ul className="glass-table divide-y divide-neutral-200">
      {rows.map((a) => (
        <li key={a.id} className="flex items-center justify-between px-4 py-3 text-body">
          ...
          <span className="text-small text-neutral-600">{new Date(a.createdAt).toLocaleString("en-US")}</span>
        </li>
      ))}
    </ul>
  );
