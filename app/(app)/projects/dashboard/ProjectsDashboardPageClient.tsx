...
export type ProjectsDashboardInitialData = {
  projects: Project[];
  taskCounts: Record<string, number>;
  allTasks: (Task & { projectId: string })[];
  sprints: Sprint[];
  activity: AuditEntry[];
};

export function ProjectsDashboardPageClient({ initial }: { initial?: ProjectsDashboardInitialData }) {
  const { selectedOrgId, loading: orgLoading } = useOrg();
  const [projects, setProjects] = useState<Project[]>(initial?.projects ?? []);
  const [taskCounts, setTaskCounts] = useState<Record<string, number>>(initial?.taskCounts ?? {});
  const [allTasks, setAllTasks] = useState<(Task & { projectId: string })[]>(initial?.allTasks ?? []);
  const [sprints, setSprints] = useState<Sprint[]>(initial?.sprints ?? []);
  const [activity, setActivity] = useState<AuditEntry[]>(initial?.activity ?? []);
