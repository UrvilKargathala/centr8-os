...
export type Contact = { id: string; fullName: string; isPrimaryContact?: boolean; isDecisionMaker?: boolean };
export type Employee = { id: string; fullName: string };
...
export type DealDetailInitialData = {
  deal: Deal;
...
export default function DealDetailPageClient({ initial }: { initial?: DealDetailInitialData }) {
  const { id } = useParams<{ id: string }>();
  const { selectedOrgId, can, loading: orgLoading } = useOrg();
  const { show: showToast } = useToast();
  const [deal, setDeal] = useState<Deal | null>(initial?.deal ?? null);
  const [account, setAccount] = useState<Account | null>(initial?.account ?? null);
  const [contact, setContact] = useState<Contact | null>(initial?.contact ?? null);
  const [stageHistory, setStageHistory] = useState<StageHistory[]>(initial?.stageHistory ?? []);
  const [activities, setActivities] = useState<Activity[]>(initial?.activities ?? []);
  const [employees, setEmployees] = useState<Employee[]>([]);
  const [accountContacts, setAccountContacts] = useState<Contact[]>(initial?.accountContacts ?? []);
  const [loading, setLoading] = useState(!initial);
  const [tab, setTab] = useState<(typeof TABS)[number]>("Overview");
  const [editForm, setEditForm] = useState<Deal | null>(initial?.deal ?? null);
      ...
      <Card>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 border-t border-neutral-200 pt-3 text-small sm:grid-cols-4">
          ...
          <div>
            ...
            <p className="text-neutral-950">{new Date(deal.createdAt).toLocaleDateString("en-US")}</p>
          </div>
          <div>
              ...
              {stageHistory.map((h) => (
                <div key={h.id} className="relative">
                  ...
                  <p className="text-caption text-neutral-500">
                    {new Date(h.changedAt).toLocaleString("en-US")} · {employeeName(h.changedBy)}
                  </p>
