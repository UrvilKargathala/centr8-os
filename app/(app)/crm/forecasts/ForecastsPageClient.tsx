...
export type ForecastsInitialData = {
...
// Mirrors buildPeriods' bucketing in lib/api/crm.ts's buildForecastPeriods,
// but only for the current period (no need to duplicate a full N-period
// loop here) — exported so the server page can compute the same period.
export function currentPeriod(periodType: PeriodType) {
  const now = new Date();
...
export function ForecastsPageClient({ initial }: { initial?: ForecastsInitialData }) {
  const router = useRouter();
  const { selectedOrgId, can, loading: orgLoading } = useOrg();
  const [periodType, setPeriodType] = useState<PeriodType>(initial?.periodType ?? "monthly");
  const [ownerId, setOwnerId] = useState("");
  const [forecast, setForecast] = useState<Forecast | null>(initial?.forecast ?? null);
  const [byRep, setByRep] = useState<ByRep[]>(initial?.byRep ?? []);
  const [trend, setTrend] = useState<TrendPoint[]>(initial?.trend ?? []);
  ...
  }, [selectedOrgId, periodType, ownerId]);
