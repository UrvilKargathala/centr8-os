...
export type Metrics = { leads_count: number; deals_count: number; revenue_won: number; roi_percent: number | null };
export type Employee = { id: string; fullName: string };

export type CampaignsInitialData = {
  campaigns: Campaign[];
  metricsById: Record<string, Metrics>;
  stats: Stats;
  employees: Employee[];
};

export function CampaignsPageClient({ initial }: { initial?: CampaignsInitialData }) {
  const router = useRouter();
  const { selectedOrgId, can, loading: orgLoading } = useOrg();
  const [campaigns, setCampaigns] = useState<Campaign[]>(initial?.campaigns ?? []);
  const [metricsById, setMetricsById] = useState<Record<string, Metrics>>(initial?.metricsById ?? {});
