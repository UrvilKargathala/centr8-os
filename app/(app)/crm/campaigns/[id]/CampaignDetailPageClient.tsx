...
export type Lead = { id: string; fullName: string; companyName: string | null; status: string; score: number | null; ownerId: string | null; createdAt: string };
export type Deal = { id: string; name: string; accountId: string | null; value: number | null; currency: string; stage: string; probability: number | null; expectedCloseDate: string | null };
export type Account = { id: string; name: string };
export type Employee = { id: string; fullName: string };

export type CampaignDetailInitialData = { detail: CampaignDetail; leads: Lead[]; deals: Deal[] };

const TABS = ["Overview", "Leads", "Deals", "AI Insights"] as const;

export default function CampaignDetailPageClient({ initial }: { initial?: CampaignDetailInitialData }) {
  const { id } = useParams<{ id: string }>();
  const router = useRouter();
  const { selectedOrgId, can, loading: orgLoading } = useOrg();
  const { show: showToast } = useToast();
  const [detail, setDetail] = useState<CampaignDetail | null>(initial?.detail ?? null);
  ...
  const [editForm, setEditForm] = useState<Campaign | null>(initial?.detail.campaign ?? null);
