...
type Lead = { id: string; fullName: string; companyName: string | null; status: string; createdAt: string; source: string | null };
...
type PipelineStage = { stage: string; count: number; total_value: number; avg_days_in_stage: number };
type PipelineStats = {
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <div>
                ...
                <p className="text-body-medium font-semibold text-neutral-950">₹{pipeline.total_pipeline_value.toLocaleString()}</p>
              </div>
              <div>
                <p className="text-caption text-neutral-600">Weighted Pipeline</p>
                <p className="text-body-medium font-semibold text-neutral-950">₹{Math.round(pipeline.weighted_pipeline_value).toLocaleString()}</p>
              </div>
              <div>
              ...
              {pipeline.stages.map((s) => (
                ...
                >
                  ...
                  <span className="w-24 shrink-0 text-right text-caption text-neutral-600">
                    {s.count} · ₹{s.total_value.toLocaleString()}
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="flex items-end gap-2">
            {dayBuckets.map((b, i) => (
              <div
                ...
                title={`${b.date.toLocaleDateString(undefined, { weekday: "long", month: "short", day: "numeric" })}: ${b.count} activit${b.count === 1 ? "y" : "ies"}`}
              >
