...
type Integration = { id: string; provider: string; status: string; connectedAt: string | null; teamName: string | null };
...
function IntegrationsPageInner() {
  ...
  const slackCallbackStatus = searchParams.get("slack");
  const slackCallbackMessage = searchParams.get("message");
  ...
  return (
    <div className="space-y-6">
      ...
      {slackCallbackStatus === "connected" && (
        <p className="rounded-md bg-success-100 p-3 text-body text-success-600">Slack connected successfully.</p>
      )}
      {slackCallbackStatus === "error" && (
        <p className="rounded-md bg-danger-100 p-3 text-body text-danger-600">
          Slack connection failed{slackCallbackMessage ? `: ${slackCallbackMessage}` : "."}
        </p>
      )}
        ...
        {PROVIDERS.map((provider) => {
          ...
          return (
            <Card key={provider.key} padding="md" className="flex flex-col gap-3">
              ...
              {connected?.teamName && <p className="text-small text-neutral-500">Workspace: {connected.teamName}</p>}
