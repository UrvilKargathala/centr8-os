...
export default function IntegrationsPageClient({ initial }: { initial?: Integration[] }) {
  return (
    <Suspense>
      <IntegrationsPageInner initial={initial} />
    </Suspense>
  );
}

function IntegrationsPageInner({ initial }: { initial?: Integration[] }) {
          ...
          return (
            <Card key={provider.key} padding="md" className="flex flex-col gap-3">
              ...
              {connected?.lastSyncedAt && (
                <p className="text-caption text-neutral-400">Last synced: {new Date(connected.lastSyncedAt).toLocaleString("en-US")}</p>
              )}
