...
export type ExecutiveInitialData = { data: DashData; health: HealthSnapshot[] };

export function ExecutivePageClient({ initial }: { initial?: ExecutiveInitialData }) {
  const { selectedOrgId, loading: orgLoading } = useOrg();
  const { increment: incrementAi, cache: aiCache } = useAiUsage();
  const [data, setData] = useState<DashData | null>(initial?.data ?? null);
  const [health, setHealth] = useState<HealthSnapshot[]>(initial?.health ?? []);
