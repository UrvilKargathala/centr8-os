...
export type Employee = { id: string; fullName: string; employmentStatus: string };
export type AttendanceRecord = { employeeId: string; checkIn: string | null; checkOut: string | null; status: string };
export type LeaveRequest = { status: string };

export type HrDashboardInitialData = {
  employees: Employee[];
  pendingLeave: number;
...
export default function HrDashboardPageClient({ initial }: { initial?: HrDashboardInitialData }) {
  const { selectedOrgId, can, loading: orgLoading } = useOrg();
  const canViewAttendance = can("attendance", "view_own");

  const [employees, setEmployees] = useState<Employee[]>(initial?.employees ?? []);
  const [todayRecords, setTodayRecords] = useState<AttendanceRecord[]>(initial?.todayRecords ?? []);
  const [weekCounts, setWeekCounts] = useState<Record<string, number>>(initial?.weekCounts ?? {});
  const [pendingLeave, setPendingLeave] = useState(initial?.pendingLeave ?? 0);
