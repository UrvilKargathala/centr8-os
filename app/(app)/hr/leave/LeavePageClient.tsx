...
export type MyLeaveInitialData = { balances: BalanceEntry[]; requests: LeaveRequest[] };

export function LeavePageClient({ initialMyLeave }: { initialMyLeave?: MyLeaveInitialData }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === "My Leave" && (
        <MyLeaveTab orgId={selectedOrgId} refreshKey={refreshKey} onRequestLeave={() => setShowRequest(true)} initial={initialMyLeave} />
      )}
...
function MyLeaveTab({
  ...
  refreshKey: number;
  onRequestLeave: () => void;
  initial?: MyLeaveInitialData;
}) {
  const [balances, setBalances] = useState<BalanceEntry[]>(initial?.balances ?? []);
