...
export function OkrsPageClient({ initialMyOkrs }: { initialMyOkrs?: Okr[] }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <OkrList
        ...
        initialMy={tab === "my" ? initialMyOkrs : undefined}
      />
...
function OkrList({
  ...
  const [okrs, setOkrs] = useState<Okr[]>(initialMy ?? []);
  const [employees, setEmployees] = useState<Employee[]>([]);
  const [loading, setLoading] = useState(!initialMy);
  ...
  const skippedInitialLoad = useRef(!!initialMy);
  useEffect(() => {
  ...
  }, [orgId, scope, refreshKey, ownEmployeeId]);
