...
export type AttendanceSettingsData = {
  ...
  workdayHoursTarget?: number | null;
  minHoursForFullDay?: number | null;
  minHoursForHalfDay?: number | null;
  weekendDays?: string[] | null;
  requireLocation?: boolean | null;
  requireNoteOnLateCheckin?: boolean | null;
  lateCheckinThresholdMinutes?: number | null;
};

function seedFrom(s?: AttendanceSettingsData | null) {
  return {
    startTime: s?.workdayStartTime?.slice(0, 5) ?? "09:00",
    endTime: s?.workdayEndTime?.slice(0, 5) ?? "18:00",
    targetHours: String(s?.workdayHoursTarget ?? 8.0),
    minFullDay: String(s?.minHoursForFullDay ?? 7.0),
    minHalfDay: String(s?.minHoursForHalfDay ?? 4.0),
    weekendDays: s?.weekendDays ?? ["saturday", "sunday"],
    requireLocation: Boolean(s?.requireLocation),
    requireNote: Boolean(s?.requireNoteOnLateCheckin),
    threshold: String(s?.lateCheckinThresholdMinutes ?? 15),
  };
}

export default function AttendanceSettingsPageClient({ initial }: { initial?: AttendanceSettingsData | null }) {
  ...
  const [targetHours, setTargetHours] = useState(seeded.targetHours);
  const [minFullDay, setMinFullDay] = useState(seeded.minFullDay);
  const [minHalfDay, setMinHalfDay] = useState(seeded.minHalfDay);
  const [weekendDays, setWeekendDays] = useState<string[]>(seeded.weekendDays);
  const [requireLocation, setRequireLocation] = useState(seeded.requireLocation);
  const [requireNote, setRequireNote] = useState(seeded.requireNote);
  const [threshold, setThreshold] = useState(seeded.threshold);
  const [loading, setLoading] = useState(initial === undefined);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const skippedInitialLoad = useRef(initial !== undefined);
  useEffect(() => {
