...
export type JobDetailInitialData = { job: JobPosting | null; candidates: Candidate[]; employees: Employee[] };

export default function JobDetailPageClient({ initial }: { initial?: JobDetailInitialData }) {
  const { job_id } = useParams<{ job_id: string }>();
  const { selectedOrgId, can, loading: orgLoading } = useOrg();
  const canCreateJob = can("recruitment", "create_job");
  const canManageCandidates = can("recruitment", "manage_candidates");
  const [job, setJob] = useState<JobPosting | null>(initial?.job ?? null);
