...
export type Period = { period_start: string; period_end: string; label: string };
...
export type Employee = { id: string; fullName: string; departmentId: string | null; departmentName: string | null };

export type PayrollInitialData = { periods: Period[]; employees: Employee[]; records: PayslipRecord[] };
...
export function PayrollPageClient({ initial }: { initial?: PayrollInitialData }) {
  ...
  const [selectedPeriod, setSelectedPeriod] = useState(
    initial?.periods[0] ? `${initial.periods[0].period_start}|${initial.periods[0].period_end}` : "",
  );
  const [records, setRecords] = useState<PayslipRecord[]>(initial?.records ?? []);
  ...
  const skippedInitialMeta = useRef(!!initial);
  useEffect(() => {
    if (!selectedOrgId || !hasAnyAccess) return;
    if (skippedInitialMeta.current) {
      skippedInitialMeta.current = false;
      return;
    }
  ...
  const skippedInitialRecords = useRef(!!initial);
  useEffect(() => {
    if (skippedInitialRecords.current) {
      skippedInitialRecords.current = false;
      return;
    }
    loadRecords();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedOrgId, periodStart, periodEnd]);
