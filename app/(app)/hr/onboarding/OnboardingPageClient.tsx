...
export type Workflow = { id: string; employeeId: string; steps: Step[]; status: string };
export type Template = { id: string; orgId: string | null; name: string; structure: { description?: string; steps?: Step[] } };
export type Employee = { id: string; fullName: string };

export type OnboardingInitialData = { workflows: Workflow[]; employees: Employee[] };

const TABS = ["Active Onboarding", "Templates"] as const;
type Tab = (typeof TABS)[number];

export default function OnboardingPageClient({ initial }: { initial?: OnboardingInitialData }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === "Active Onboarding" && <ActiveOnboardingTab orgId={selectedOrgId} initial={initial} />}
      {tab === "Templates" && <TemplatesTab orgId={selectedOrgId} canConfigure={can("onboarding", "configure")} />}
    </div>
  );
}

function ActiveOnboardingTab({ orgId, initial }: { orgId: string; initial?: OnboardingInitialData }) {
