...
export type ActiveSurveysInitialData = { surveys: Survey[]; responded: Record<string, boolean> };

export function SurveysPageClient({ initialActive }: { initialActive?: ActiveSurveysInitialData }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === "Active Surveys" && (
        <ActiveSurveysTab orgId={selectedOrgId} refreshKey={refreshKey} onChanged={() => setRefreshKey((k) => k + 1)} initial={initialActive} />
      )}
...
function ActiveSurveysTab({
  ...
  refreshKey: number;
  onChanged: () => void;
  initial?: ActiveSurveysInitialData;
}) {
  const [surveys, setSurveys] = useState<Survey[]>(initial?.surveys ?? []);
  const [responded, setResponded] = useState<Record<string, boolean>>(initial?.responded ?? {});
