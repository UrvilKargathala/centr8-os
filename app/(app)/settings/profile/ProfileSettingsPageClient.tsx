...
export type SecurityLogEntry = { id: string; action: string; createdAt: string; targetType: string };

export type ProfileSettingsInitialData = { profile: ProfileResponse; securityLog: SecurityLogEntry[] };
...
export default function ProfileSettingsPageClient({ initial: initialData }: { initial?: ProfileSettingsInitialData }) {
  const router = useRouter();
  const { selectedOrgId, loading: orgLoading } = useOrg();
  const toast = useToast();
  const seededDrafts = initialData ? seedDrafts(initialData.profile.preferences) : null;
  const [data, setData] = useState<ProfileResponse | null>(initialData?.profile ?? null);
  ...
  const [account, setAccount] = useState(seededDrafts?.account ?? { fullName: "", jobTitle: "", department: "", phone: "", timezone: "UTC", language: "en" });
  ...
  const initial = useRef<{ account: typeof account; prefs: typeof prefs; notify: typeof notify } | null>(seededDrafts ?? null);

  const [saving, setSaving] = useState<string | null>(null);
  const [pwModal, setPwModal] = useState(false);
  const [deleteModal, setDeleteModal] = useState(false);
  const [securityLog, setSecurityLog] = useState<SecurityLogEntry[]>(initialData?.securityLog ?? []);
  ...
  function loadAll() {
    ...
    fetch(`/api/me/profile?org_id=${selectedOrgId}`)
      ...
      .then((b) => {
        ...
        setData(b.data);
        const seeded = seedDrafts(b.data.preferences);
        setAccount(seeded.account);
        setPrefs(seeded.prefs);
        setNotify(seeded.notify);
  ...
  // MFA factors and session info always need a client-side Supabase call
  // (auth.mfa.listFactors / auth.getUser) — not seeded server-side, same as
  // every other Supabase-auth-direct read in this app.
  useEffect(() => {
              ...
              <ul className="glass-card rounded-md divide-y divide-neutral-200">
                {securityLog.map((row) => (
                  <li key={row.id} className="flex items-center justify-between px-3 py-2 text-body">
                    ...
                    <span className="text-small text-neutral-500">{new Date(row.createdAt).toLocaleString("en-US")}</span>
                  </li>
                ))}
              </ul>
            )}
