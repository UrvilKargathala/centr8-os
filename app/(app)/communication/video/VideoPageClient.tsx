...
export type VideoInitialData = { connected: boolean; upcoming: GoogleMeeting[]; past: GoogleMeeting[] };
...
export function VideoPageClient({ initial }: { initial?: VideoInitialData }) {
  ...
  const [upcoming, setUpcoming] = useState<GoogleMeeting[]>(initial?.upcoming ?? []);
  const [past, setPast] = useState<GoogleMeeting[]>(initial?.past ?? []);
  ...
  const skippedInitialMeetings = useRef(!!initial);
  useEffect(() => {
    if (!connected) return;
    if (skippedInitialMeetings.current) {
      skippedInitialMeetings.current = false;
      return;
    }
    loadMeetings();
                      ...
                      <li key={m.id} className="glass-card p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <p className="mt-0.5 text-caption text-neutral-500">{new Date(m.startTime).toLocaleString("en-US")}</p>
                          </div>
                          {tab === "upcoming" && (
