...
export type MailInitialData = { connected: boolean; messages: GmailMsg[]; nextPageToken: string | null };

export function MailPageClient({ initial }: { initial?: MailInitialData }) {
  const { selectedOrgId } = useOrg();
  const [tab, setTab] = useState<(typeof TABS)[number]>("Inbox");
  const [q, setQ] = useState("");
  const [messages, setMessages] = useState<GmailMsg[]>(initial?.messages ?? []);
  const [loading, setLoading] = useState(!initial);
  const [error, setError] = useState<string | null>(null);
  const [connected, setConnected] = useState(initial?.connected ?? true);
  const [nextPageToken, setNextPageToken] = useState<string | null>(initial?.nextPageToken ?? null);
                    ...
                    <div key={m.id} className="flex gap-3 glass-card rounded-md p-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-baseline justify-between">
                          ...
                          <span className="text-caption text-neutral-500">{new Date(m.date).toLocaleString("en-US")}</span>
                        </div>
