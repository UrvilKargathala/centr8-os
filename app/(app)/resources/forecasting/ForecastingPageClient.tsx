...
export type ForecastingInitialData = { summary: SummaryData | null; deptUtil: DeptUtilRow[] };

export default function ForecastingPageClient({ initial }: { initial?: ForecastingInitialData }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === "summary" && <SummaryTab orgId={selectedOrgId} initial={initial} />}
...
function SummaryTab({ orgId, initial }: { orgId: string; initial?: ForecastingInitialData }) {
  const [period, setPeriod] = useState("quarter");
  const [summary, setSummary] = useState<SummaryData | null>(initial?.summary ?? null);
  const [insights, setInsights] = useState<AiInsights | null>(null);
  const [insightsLoading, setInsightsLoading] = useState(false);
  const [deptUtil, setDeptUtil] = useState<DeptUtilRow[]>(initial?.deptUtil ?? []);
