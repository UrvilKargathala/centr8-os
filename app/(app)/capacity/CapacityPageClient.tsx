...
export type Task = { id: string; title: string; assigneeId: string | null; status: string; estimate: number | null };
...
export function CapacityPageClient({
  ...
  const skippedInitialLoad = useRef(!!initialPeople);
  useEffect(() => {
    ...
    // /api/people never existed (this page's original fetch 404'd
    // silently) and /api/tasks only supports a single status value, not a
    // comma list — /api/team?active=all and a client-side status filter
    // are the real equivalents, matching lib/api/team.ts's listCapacityData
    // used for the initial server-rendered load.
    const OPEN_STATUSES = new Set(["todo", "in_progress", "in_review"]);
    Promise.all([
      fetch(`/api/team?org_id=${selectedOrgId}&active=all`).then((r) => r.json()),
      fetch(`/api/tasks?org_id=${selectedOrgId}`).then((r) => r.json()),
    ])
      .then(([pRes, tRes]) => {
        setPeople(pRes.data ?? []);
        setTasks(((tRes.data ?? []) as Task[]).filter((t) => OPEN_STATUSES.has(t.status)));
      })
