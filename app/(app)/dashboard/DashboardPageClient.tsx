...
export function DashboardPageClient({ initialData }: { initialData?: DashboardData | null }) {
  ...
  // Explicit locale (not `undefined`) — this page is now server-rendered,
  // and an implicit locale resolves differently on the Node server vs. the
  // browser, causing a hydration text mismatch that a purely client-rendered
  // page never hit.
        ...
        {data.deals ? (
          <CardLink href="/crm/deals" className={KPI_CARD}>
            ...
            <div>
              ...
              <div className="flex items-baseline gap-2">
                <p className="text-h3 font-semibold leading-tight text-neutral-950">₹{Math.round(data.deals.weighted_pipeline_value).toLocaleString("en-US")}</p>
                  ...
                  <a key={p.id} href={`/projects/${p.id}`} className="flex items-center gap-3 py-2.5 first:pt-0 last:pb-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-caption text-neutral-500">
                        ...
                        {p.end_date ? ` · due ${new Date(p.end_date).toLocaleDateString("en-US")}` : ""}
                      </p>
                    </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-neutral-100 pt-3">
                  <div>
                    ...
                    <p className="text-body-medium font-semibold text-neutral-950">₹{data.deals.open_pipeline_value.toLocaleString("en-US")}</p>
                  </div>
                  <div>
                    <p className="text-caption text-neutral-600">Weighted Pipeline</p>
                    <p className="text-body-medium font-semibold text-neutral-950">
                      ₹{Math.round(data.deals.weighted_pipeline_value).toLocaleString("en-US")}
                    </p>
                  </div>
                </div>
              </div>
            ...
            {meeting ? (
              <>
                ...
                <p className="mt-1 text-small text-neutral-400">
                  {new Date(meeting.startTime).toLocaleString("en-US", { weekday: "short", hour: "numeric", minute: "2-digit" })}
                </p>
                {meeting.meetUrl ? (
                  <a
