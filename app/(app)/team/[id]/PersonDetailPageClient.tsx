...
export type PersonDetailInitialData = { person: Person; stats: Stats };
...
export default function PersonDetailPageClient({ initial }: { initial?: PersonDetailInitialData }) {
  const params = useParams<{ id: string }>();
  const router = useRouter();
  const id = params.id;
  const [person, setPerson] = useState<Person | null>(initial?.person ?? null);
  ...
  const joined = new Date(person.createdAt).toLocaleDateString("en-US", { month: "long", year: "numeric" });
                  ...
                  return (
                    <li key={t.id} className="flex items-center justify-between gap-2 py-2">
                      <div className="min-w-0">
                        ...
                        <p className="text-caption text-neutral-500">{new Date(t.updatedAt).toLocaleDateString("en-US")}</p>
                      </div>
