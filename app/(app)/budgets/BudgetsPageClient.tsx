...
export type Project = { id: string; name: string; status: string; budgetAllocated: number | null; budgetSpent: number | null };

function formatCurrency(value: number) {
  // Explicit locale (not `undefined`) — this page is server-rendered, and an
  // implicit locale resolves differently on the Node server vs. the browser,
  // causing a hydration mismatch a purely client-rendered page never hit.
...
export function BudgetsPageClient({ initialProjects }: { initialProjects?: Project[] }) {
  ...
  const skippedInitialLoad = useRef(!!initialProjects);
  useEffect(() => {
